...
All possible combinations (APC) of exons and introns are generated from a given DNA sequence. Introns are gathered based only on canonical donor and acceptor sites. A modified backtracking algorithm is used to generate only valid combinations of introns (isoforms), for example introns that overlap would be excluded from the output. This reduces the search space. **Need graph showing algorithm performance**
...
start by reading in intron sequences with reticulate
...
NEED TO DO 
Genome stats-how many genes are there for each intron count? Is 4 introns good enough for APC?
Average structure of gene in wormbase
