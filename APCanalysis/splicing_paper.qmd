...
Transcripts on WB have varrying support for each intron. What is that variation?
In a single transcript, intron counts should be the same. But what is the actual manhattan distance within a transcript?
All pairwise comparisons between introns in a single transcript?
Compare introns 1, 2, 3, 4...
1/2 ratio difference?
Expected deviation?
Expected manhattan distance?
Compare intron space where all are 1:1:1:1
only introns that match the canonical isoform
Not raw counts instead ratios, 0.25 vs 0.35? 
Do on all genes and all transcripts
Ratio of transcript distribution
